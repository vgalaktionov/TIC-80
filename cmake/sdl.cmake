...
if(BUILD_SDL)
    ...
    if(BUILD_SDLGPU)
        ...
        option(BUILD_CRT_FAST "Use linear-light filtered CRT on desktop Linux" OFF)
        option(BUILD_LOW_LATENCY "Complete GPU work before presentation on desktop Linux" OFF)
        if(BUILD_CRT_FAST OR BUILD_LOW_LATENCY)
            if(NOT LINUX OR ANDROID OR RPI)
                message(FATAL_ERROR "Optimized rendering options require desktop Linux OpenGL")
            endif()
            find_package(OpenGL REQUIRED)
            target_link_libraries(${TIC80_TARGET} OpenGL::GL)
        endif()
        if(BUILD_LOW_LATENCY)
            target_compile_definitions(${TIC80_TARGET} PRIVATE TIC80_LOW_LATENCY)
        endif()
        if(BUILD_CRT_FAST)
            target_sources(${TIC80_TARGET} PRIVATE src/system/sdl/crt-fast.c)
            target_compile_definitions(${TIC80_TARGET} PRIVATE TIC80_CRT_FAST)
        endif()
    else()
        if(EMSCRIPTEN)
        elseif(RPI)
